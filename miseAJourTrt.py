...
def miseAJour(host,user,password,nomBD,port,ldapServer,ldapUsername, ldapPassword,listeEtab,listeEtabSansAdmin,listeEtabSansMail,structures_dn,personnes_dn,entete,prefixAdminMoodle, prefixAdminLocal, EtabRgp, NomEtabRgp, UaiRgp, fileTrtPrecedent, fileSeparator, purgeCohortes):
    try:
                ...
                if gereAdminLocal:
                    for member in enseignant_is_member_of:
                        ...
                        if adminMoodle:
                            ...
                            if insert:
                                logging.info("      |_ Insertion d'un admin  local %s %s %s" % (enseignant_uid, enseignant_given_name, enseignant_sn))
                            ...
                            if delete:
                                logging.info("      |_ Suppression d'un admin local %s %s %s" % (enseignant_uid, enseignant_given_name, enseignant_sn))
	    ...
	    create_profs_etabs_cohorts( mark, entete, id_context_categorie, uai_etablissement, maintenant_sql, time_stamp, ldapServer, ldapUsername, ldapPassword, personnes_dn  )
            ####################################
        ...
        for ldap_entry in result_set :
                ...
                for member in people_is_member_of:
                    ...
                    if admin:
                        ...
                        delete = delete_moodle_local_admin(mark, entete, id_context_categorie_inter_etabs, id_user)
                        if delete:
                            logging.info("      |_ Suppression d'un admin local %s %s %s" % (people_uid, people_given_name, people_sn))
...
#
######################################################################
def miseAJourInspecteurs( host, user, password, nomBD, port, ldapServer, ldapUsername, ldapPassword, personnes_dn, structures_dn, entete, ldap_attribut_user, ldap_valeur_attribut_user ):
    try:
        logging.info('============================================')
